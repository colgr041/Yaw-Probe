...
void setup() {
  ...
  nextSampleTime = millis(); // initialize scheduler
}

void loop() {
  // Keep IoT Cloud alive
  ArduinoCloud.update();

  // ✅ Precise 1 Hz sampling (no drift)
  if (millis() >= nextSampleTime) {
    ...
    // Update IoT Cloud variable (latest value)
    pressure = pa;
...
void sendBatch() {
  if (client.connect(host, 443)) {
    ...
    Serial.println("Sent batch to Google");
